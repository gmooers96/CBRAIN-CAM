...
def anneal_schedule(epoch):
    #print("This is a test print statement", min(epoch * 0.01, 1.))
    #print("This is a test", k_start)
    return min(epoch * 0.01, 1.)
...
def kl(z_mean, z_log_var):
    def _kl(true, pred):
        ...
        #return hp_lambda
        return K.mean(kl_loss)*hp_lambda
...
def main():
    ...
    ###
    # add in the anneal stuff
    #weight = model_config["loss"]["start_weight"]
    klstart = model_config["loss"]["klstart"]        
    #annealtime = model_config["loss"]["kl_annealtime"]  
    ###
    ...
    vae.compile(
        ...
        metrics=[
            ...
            kl_reconstruction_loss(encoder_result.z_mean, encoder_result.z_log_var)
        ]
    )
    vae.summary()
    ...
    h = vae.fit(
        ...
        batch_size=model_config["batch_size"], 
        validation_data=[test_data, test_data],
        callbacks=[AneelingCallback(anneal_schedule, hp_lambda), callbacks_list]
    )
