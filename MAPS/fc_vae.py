...
def kl_reconstruction_loss(z_mean, z_log_var, kl_weight):
...
def main():
    ...
    #optimizer = keras.optimizers.RMSprop(lr=model_config['optimizer']['lr'])
    optimizer = keras.optimizers.Adam(lr=model_config['optimizer']['lr'])

    # Compile model 
    vae.compile(
        # loss=reconstruction, 
        loss=kl_reconstruction_loss(encoder_result.z_mean, encoder_result.z_log_var, model_config["loss"]["kl_weight"]), 
        optimizer=optimizer, 
        metrics=[
            reconstruction, 
            kl(encoder_result.z_mean, encoder_result.z_log_var), 
            kl_reconstruction_loss(encoder_result.z_mean, encoder_result.z_log_var, model_config["loss"]["kl_weight"])
        ]
    )
    vae.summary()
