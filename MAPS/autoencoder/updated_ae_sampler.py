...
def sample_frob_norm(vae, test_data, id, dataset_max, dataset_min): 
    ...
    preds = np.squeeze(sample_mean)
    preds = np.reshape(preds, (len(test_data),30*128))
    print("preds shape is", preds.shape)
    print("truths shape is", truths.shape)

    Rough_Metric = f_norm(truths, preds)
...
def sample_latent_space(vae_encoder, train_data, test_data, id, dataset_min, dataset_max, test_labels, dataset_type): 
    ...
    z_test = vae_encoder.predict(test_data)
    z_train = vae_encoder.predict(train_data)
    ...
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/autoencoder/Saved_Data/Latent_Space__{}".format(id), z_test_tsne)
    if dataset_type == "half_deep_convection":
        ...
        junk = np.squeeze(z_test_tsne[np.where(test_labels == 2),:])
        ...
        plt.scatter(x=junk[:, 0], y=junk[:, 1], c="yellow", s=0.4, label="Red Noise")
        plt.legend()
    ...
    plt.savefig('./model_graphs/latent_space/Red_Noisy_Image_latent_space_with_pca_binary_{}.png'.format(id))
...
def main():
    ...
    sample_frob_norm(vae, test_data, args.id, dataset_max, dataset_min)
